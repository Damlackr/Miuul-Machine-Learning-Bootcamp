...
df["age_flag"]=df["age"].apply(otuzyas)
df["age_flag"]
...
ndf = df[(df["time"] == "Lunch") & (df["sex"] == "Female")]
ndf.groupby(["day"]).agg({"total_bill": ["sum", "min", "max", "mean"],"tip": ["sum", "min", "max", "mean"]})

#Görev 21: size'i 3'ten küçük, total_bill'i 10'dan büyük olan siparişlerin ortalaması nedir? (loc kullanınız).

df.loc[(df["size"] < 3) & (df["total_bill"] > 10)].mean()

#Görev 22: total_bill_tip_sum adında yeni bir değişken oluşturunuz. Her bir müşterinin ödediği totalbill ve tipin
# toplamını versin.

df["total_bill_tip_sum"] = df["total_bill"] + df["tip"]

#Görev 23: Total_bill değişkeninin kadın ve erkek için ayrı ayrı ortalamasını bulunuz.
# Bulduğunuz ortalamaların altında olanlara 0, üstünde ve eşit
...
def total_bill_flag(sex, total_bill):
    kadınort = df["total_bill"].loc[df["sex"] == "Female"].mean()
    erkekort= df["total_bill"].loc[df["sex"] == "Male"].mean()
    if sex == "female":
        if total_bill < kadınort:
            return 0
        else:
            return 1
    else:
        if total_bill < erkekort:
            return 0
        else:
            return 1
...
df.groupby(["total_bill_flag","sex"]).total_bill_flag.value_counts()
...
dfn.sort_values(by=["total_bill_tip_sum"],ascending=False).head(30)